...
interface AgentStateEntry {
  ...
  lifecycle: AgentLifecycle     // 'persistent' | 'scheduled' | 'ephemeral'
  ...
  task: string | null           // task description (ephemeral/scheduled agents)
...
Spawned and killed on a cron schedule. May self-exit early if work completes.

**Spawn:** Cron calls `spawn_teammate` with `lifecycle: 'scheduled'`. Daemon creates a channel.

**Death:** Cron calls `kill_teammate` at the end of the window, or agent self-exits (completion protocol). The scheduled kill is the safety net.

**Cron config** (in `agents/<name>/crons.md`):
```
spawn: 25 9 * * 1-5   # 9:25am Mon-Fri
kill:  5 16 * * 1-5   # 4:05pm Mon-Fri
```

**reconcile behavior:** If found dead outside its schedule window → mark dead and prune. If within window → respawn.
...
3. For each alive agent in daemon:
   a. No tmux window → stale entry
      ...
      - lifecycle == 'scheduled': check schedule window; respawn or POST /agent/died (if --fix)
